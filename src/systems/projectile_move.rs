    ...
    fn run(
            ...
            (&projectiles, &mut transforms, &speeds, &directions).join()
        {
            ...
            match direction.current_direction {
                Direction::Up => {
                    transform.set_translation_y(projectile_loc.y + (speed.speed * time.delta_seconds()));
                }
                Direction::Down => {
                    transform.set_translation_y(projectile_loc.y - (speed.speed * time.delta_seconds()));
                }
                Direction::Right => {
                    transform.set_translation_x(projectile_loc.x + (speed.speed * time.delta_seconds()));
                }
                Direction::Left => {
                    transform.set_translation_x(projectile_loc.x - (speed.speed * time.delta_seconds()));
                }
